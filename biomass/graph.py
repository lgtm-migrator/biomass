...
class NetworkGraph(object):
    ...
    def observables(self) -> List[str]:
        duplicate = [
            name
            for name in set(self.problem.obs_names)
            if self.problem.obs_names.count(name) > 1
        ]
        if not duplicate:
    ...
    def _extract_equation(
        dir: str, filepath: str, left_match: str, right_match: str
    ) -> dict:
        """
    ...
    def to_graph(self) -> None:
        ...
        if not use_flux:
            left_re = r"(?<=dydt\[V.)(.+?)(?=\])"
            right_re = r"(?<=y\[V.)(.+?)\]"
            edges = self._extract_equation(
                self.path, "ode.py", left_re, right_re
            )
        else:
            left_re_flux = r"(?<=v\[)(.+?)(?=\])"
            right_re_flux = r"(?<=y\[V.)(.+?)\]"
            ...
            odes = self._extract_equation(
                self.path, "ode.py", left_re_ode, right_re_ode
            )
...
class Text2ModelGraph(object):
    ...
    def __init__(self, path: str, biomass_model: ModuleType):
        self._path = path
        # self._parameters = biomass_model.C.NAMES
        self._species = biomass_model.species
        # self.pval = biomass_model.param_values
        # self.ival = biomass_model.initial_values
        # self.problem = biomass_model.OptimizationProblem()
        # self.viz = biomass_model.Visualization()
        # self.rxn = biomass_model.ReactionNetwork()
        self._kinetics = biomass_model.kinetics
        self._stoichmatrix = biomass_model.stoichiometry_matrix.toarray()
    ...
    @property
    def stoichometry_matrix(self):
        return self._stoichmatrix

    def _construct_adjacency_matrix(self) -> dict:
        """
        Matches species that interact with each other by reading python files as text and looking for equations.
        """
        adj_dim = self.stoichometry_matrix.shape[0]
        adj_matrix = np.zeros((adj_dim, adj_dim))
        for reaction in self.stoichometry_matrix.transpose():
            outnodes, innodes = [], []
            for i, species in enumerate(reaction):
                if species < 0:
                    outnodes += [i]
                if species > 0:
                    innodes += [i]
            adj_matrix[innodes, outnodes] = 1
        return adj_matrix
